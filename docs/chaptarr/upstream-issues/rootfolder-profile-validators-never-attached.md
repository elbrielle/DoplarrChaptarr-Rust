...
This looks like an oversight rather than a decision, because the three other controllers
that take the same two validators do attach them.

## Observed behavior

`POST /api/v1/rootfolder` with a `qualityProfileId` or `metadataProfileId` that matches no
existing profile succeeds. The root folder is created with the bad id stored on it. Nothing
in the response indicates a problem. The failure only appears later, when something tries to
resolve the profile for that root folder.

The same holds for the per-format ids on the resource (`audiobookQualityProfileId`,
`audiobookMetadataProfileId`, `ebookQualityProfileId`, `ebookMetadataProfileId`), which are
also unvalidated.
...
- `src/Chaptarr.Api.V1/RootFolders/RootFolderResource.cs:14-15` — `QualityProfileId`, `MetadataProfileId`
...
Attach the injected validators the way `AuthorController` and `ImportListController` do, for
example:
...
and the same for the four per-format ids if they should be checked too. Because the
