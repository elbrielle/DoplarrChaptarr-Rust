...
The constraint:
- `src/NzbDrone.Core/Datastore/Migration/001_chaptarr_complete_schema.cs:380` —
  `.WithColumn("Images").AsString().WithDefaultValue("[]")`, i.e. NOT NULL with a default.
  The default only applies when the column is omitted from the insert; an explicit `NULL`
  still violates it, which matches the observed message. `Links` at `:381` has the same shape.
...
We could not confirm from source alone which of the add branches produced the failing insert
in our run, so treat the specific write path as inferred rather than proven; the constraint,
the unguarded mapping at `EditionResource.cs:188`, and the observed message are all directly
verified.
